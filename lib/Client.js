...
RealtimetrainsClient.prototype.getLocationList = function (query) {
  ...
  return this.axios.get(this.config.uri + '/json/search/' + query.station, {
    auth: {
      username: this.config.username,
      password: this.config.password
    }
...
RealtimetrainsClient.prototype.getService = function (query) {
  ...
  if (query.hasOwnProperty('date')) {
    ...
    return this.axios.get(this.config.uri + '/json/service/' + query.service + '/' + query.date)
  } else {
    return this.axios.get(this.config.uri + '/json/service/' + query.service)
  }
}
