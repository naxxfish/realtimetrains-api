...
describe('Service', () => {
  ...
  describe('getService', () => {
    ...
    test('should throw an AssertationError if the date format is incorrect', () => {
      var service = new Service(CONFIG)
      expect(() => {
        service.getService({ 'service': 'W72419', 'date': 'blah' })
      }).toThrow(AssertionError)
    })
    test('should return a service record if specified by date', () => {
      var service = new Service(CONFIG)
      axiosMock.onGet('/v1/json/service/W72419/2013/06/11').reply(200, TEST_FIXTURE)
      expect(() => {
        service.getService({ 'service': 'W72419', 'date': '2013/06/11' }).then((returnedService) => {
          expect(returnedService).toEqual(TEST_FIXTURE)
        })
      }).not.toThrow()
    })
